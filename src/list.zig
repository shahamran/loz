...
pub fn List(comptime T: type) type {
    return struct {
        ...
        pub fn reserve(self: *Self, capacity: usize) !void {
            ...
            const new_items = try self.allocator.realloc(self.allocated_slice(), new_capacity);
...
test "list deinit empty" {
    var list = List(i32).init(testing_allocator);
    list.deinit();
}
...
test "list eql" {
    var list1 = List(u8).init(testing_allocator);
    var list2 = List(u8).init(testing_allocator);
    defer list1.deinit();
    defer list2.deinit();
